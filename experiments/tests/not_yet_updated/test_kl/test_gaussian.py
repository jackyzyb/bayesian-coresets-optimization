...
def ll_m2_exact(muw, Sigw, Siginv, x):
    ...
    return 0.25 * (crho + musq * musq[:, np.newaxis] + np.diag(Rho).sum() * (musq + musq[:, np.newaxis]) + 4 * np.dot(
        np.dot(mu, Rho), mu.T))
...
tests = [(N, D, dist, algn) for N in [1, 10] for D in [1, 3] for dist in ['gauss', 'bin', 'axis_aligned'] for algn in
         algs_nms]
...
def coreset_single(N, D, dist, algn):
    ...
    for m in range(1, N + 1):
        ...
        if x.shape[0] == 1:
            assert np.fabs(
                w - np.array([1])) < tol, anm + " failed: coreset not immediately optimal with N = 1. weights: " + str(
                coreset.weights()) + " mup = " + str(mup) + " Sigp = " + str(Sigp) + " muw = " + str(
                muw) + " Sigw = " + str(Sigw)
            # check if coreset is valid
        assert (w > 0.).sum() <= m, anm + " failed: coreset size > m"
        assert (
                           w > 0.).sum() == coreset.size(), anm + " failed: sum of coreset.weights()>0  not equal to size(): sum = " + str(
        ...
        assert np.fabs(
            coreset.error() - err) < tol, anm + " failed: error est is not close to true err: est err = " + str(
    ...
    assert coreset.M == 0 and np.all(np.fabs(coreset.weights()) == 0.) and np.fabs(
        coreset.error() - err) < tol and not coreset.reached_numeric_limit, anm + " failed: reset() did not properly reset"
    ...
    if dist != 'bin':
        ...
        assert np.sqrt(((
                                    w - w_inc) ** 2).sum()) < tol, anm + " failed: incremental buid up to N doesn't produce same result as one run at N : \n error = " + str(
