...
def test_api():
    for N in [0, 1, 10]:
        ...
        assert np.all(coreset.weights() == coreset.wts), "FullDataCoreset failed: wts should be = weights(): " + str(
            coreset.wts) + " " + str(coreset.weights())
...
def test_build():
    for N in [0, 1, 10]:
        ...
        for m in [1, 10, 100]:
            ...
            assert M == N and coreset.M == N, "FullDataCoreset failed: M should = N after build. M = " + str(
                M) + " N = " + str(N) + " coreset.M = " + str(coreset.M)
