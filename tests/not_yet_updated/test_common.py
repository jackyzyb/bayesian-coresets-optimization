...
types.extend(['kl'] * 4)
algs.extend([LassoCoreset, FrankWolfeCoreset, GIGACoreset, ForwardStagewiseCoreset, MatchingPursuitCoreset, LARCoreset,
             VectorSamplingCoreset, VectorUniformSamplingCoreset, OrthoPursuitCoreset])
...
def test_coreset():
    for N, D, at in tests:
        ...
        if typ == 'dummy':
            ...
            coreset = alg(potentials=lambda x: dummy_potentials(x, N), sampler=lambda w, n: dummy_sampler(w, n, D),
                          n_samples=20, N=N)
        elif typ == 'vector':
...
def reset(N, D, coreset):
    for m in [1, 10]:
        ...
        # check reset
        coreset.reset()
        assert coreset.M == 0 and np.all(np.fabs(
            coreset.wts) == 0.) and not coreset.reached_numeric_limit, coreset.alg_name + " failed: reset() did not properly reset"
...
def build(N, D, coreset):
    for m in [1, 10]:
        ...
        assert (
                           coreset.wts > 0).sum() <= coreset.M, coreset.alg_name + " failed: number of nonzero weights must be <= M: number = " + str(
            (coreset.wts > 0).sum()) + " M = " + str(coreset.M)
        assert (
                           coreset.wts > 0).sum() == coreset.size(), coreset.alg_name + " failed: number of nonzero weights != size()"
