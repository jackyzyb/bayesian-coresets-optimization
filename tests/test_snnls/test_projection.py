...
def single_llgll(ll, gll, g2, gF, samp):
    ...
    assert np.all(
        np.fabs(num_grad - exact_grad) < 1e-6), "error: numerical/exact gradients do not match up; max diff = " + str(
        np.fabs(num_grad - exact_grad).max())
    ...
    assert np.all(np.fabs(
        num_gramF - exact_gramF) < 5e-2), "error: numerical/exact gramF matrices don't match up; max diff = " + str(
        np.fabs(num_gramF - exact_gramF).max())
    assert np.all(np.fabs(
        num_gram2 - exact_gram2) < 5e-2), "error: numerical/exact gram2 matrices don't match up; max diff = " + str(
        np.fabs(num_gram2 - exact_gram2).max())


def test_llgll():
    for ll, gll, g2, gF, samp in [(ll_linear, gll_linear, gram2_linear, gramF_linear, sample_linear),
                                  (ll_quad, gll_quad, gram2_quad, gramF_quad, sample_quad)]:
...
def single_projF(gll, gram, samp):
    ...
    w = proj.get()
    assert np.all(
        np.fabs(gram(x) - w.dot(w.T)) < 1e-2), "error: projectionF doesn't converge to expectation; max diff = " + str(
...
def single_proj2(ll, gram, samp):
    ...
    w = proj.get()
    assert np.all(
        np.fabs(gram(x) - w.dot(w.T)) < 1e-2), "error: projection2 doesn't converge to expectation; max diff = " + str(
        np.fabs(gram(x) - w.dot(w.T)).max())
    proj.reset()
    assert proj.get().shape == w.shape, "error: proj.reset() doesn't retain shape"
