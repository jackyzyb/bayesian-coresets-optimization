...
fig_cput = bkp.figure(y_axis_type='log', y_axis_label='Normalized Fisher Information Distance', x_axis_type='log',
                      x_axis_label='Relative Total CPU Time', x_range=(.05, 1.1), plot_width=1250, plot_height=1250)
fig_csz = bkp.figure(y_axis_type='log', y_axis_label='Normalized Fisher Information Distance', x_axis_type='log',
...
for idx, zppd in enumerate(dnmsalgs):
    ...
    fig_cput.line(np.percentile(cputs, 50, axis=0) / std_ts[dnm], np.percentile(Fs, 50, axis=0) / std_Fs[dnm],
                  line_color=alg[2], line_width=8, legend=alg[1])
    fig_csz.line(np.percentile(cszs, 50, axis=0), np.percentile(Fs, 50, axis=0) / std_Fs[dnm], line_color=alg[2],
                 line_width=8, legend=alg[1])

rndlbl = bkm.Label(x=1.0, x_offset=-10, y=700, y_units='screen', text='Full Dataset MCMC', angle=90, angle_units='deg',
                   text_font_size='30pt')
