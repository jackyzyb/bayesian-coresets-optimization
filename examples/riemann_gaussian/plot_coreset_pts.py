...
for m in Ms:
  ...
  if size_x_axis:
    plot_gaussian(fig, muwt[msz,:], (9./9.)*Sigwt[msz,:], (9./9.)*Sig, pal[4], 17, 9, 1, 1, 'solid', nm[1]+', size ' + str( (wt[msz, :]>0).sum() ))
  else:
    plot_gaussian(fig, muwt[m,:], (4./9.)*Sigwt[m,:], (4./9.)*Sig, pal[4], 17, 9, 1, 1, 'solid', nm[1]+', ' + str(m) +' pts')
