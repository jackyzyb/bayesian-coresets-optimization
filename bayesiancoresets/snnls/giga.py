...
class GIGA(SparseNNLS):
    def __init__(self, A, b):
        ...
        if np.any(Anorms == 0):
            raise ValueError(self.alg_name +
                             '.__init__(): A must not have any 0 columns')
    ...
    def _select(self):
        ...
        if cdirnrm < util.TOL:
            raise NumericalPrecisionError('cdirnrm < TOL: cdirnrm = ' +
                                          str(cdirnrm))
        cdir /= cdirnrm
        scorends = self.An.T.dot(
            np.hstack((cdir[:, np.newaxis], xw[:, np.newaxis])))
        #extract points for which the geodesic direction is stable (1st condition) and well defined (2nd)
        idcs = np.logical_and(scorends[:, 1] > -1. + 1e-14,
                              1. - scorends[:, 1]**2 > 0.)
        #compute the norm
    ...
    def _reweight(self, f):
        ...
        gA = self.bn.dot((xf / nf)) - self.bn.dot((xw / nw)) * (xw / nw).dot(
            (xf / nf))
        gB = self.bn.dot((xw / nw)) - self.bn.dot((xf / nf)) * (xw / nw).dot(
            (xf / nf))
        if gA <= 0. or gB < 0:
